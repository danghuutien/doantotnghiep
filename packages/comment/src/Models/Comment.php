...
class Comment extends BaseModel {
    ...
    public static function searchComment($type, $type_id, $keyword, $sort) {
    ...
    public function timeComment($time) {
        $time = isset($time) ? $time : Carbon::now();
        $now = Carbon::now();
        return \Carbon\Carbon::parse($time)->diffForHumans($now);
    }
    public function child(){
        $list_child = Comment::where('status',1)->where('parent_id',$this->id)->orderBy('id','desc')->get();
