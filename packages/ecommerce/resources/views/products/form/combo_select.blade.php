               ...
               <div class="result-full-item col col-md-12">
                    ...
                    <div class="row">
                        <div class="controls col col-md-6 col-lg-6 col-xs-6 suggest">
                            ...
                            @php
                                $productCombo = Sudo\Ecommerce\Models\Product::where('id', $value['product_id'])->first();
                            @endphp
                                <label for="product_sale_name_{{ $key }}" class="col-lg-12 col-form-label">Sản phẩm</label>
                                <input type="text" class="form-control disable" autocomplete="off" id="product_sale_name_{{ $key }}_input" disabled="" name="sussget_name" placeholder="Tìm theo tên sản phẩm" value="{{ $productCombo->name?? '' }}">
                            <input type="hidden" class="product_id" name="sale[{{ $key }}][product_id]" value="{{ $value['product_id'] }}">
                            <input type="hidden" class="variant_id" name="sale[{{ $key }}][variant_id]" value="{{ $value['variant_id'] ?? 0 }}">
                            <div class="suggest-result"><ul></ul></div>
                        </div>
                        <div class="controls col col-md-6 col-lg-6 col-xs-6">
                            ...
                            <input  type="number" id="type_{{$key}}" class="combo_price" autocomplete="off" data-max="{{ $productCombo->price ?? 0 }}" name="sale[{{$key}}][price]" value="{{$value['price'] ?? 0}}">
            ...
            if (keyword.length > 0) {
                suggest = setTimeout(function() {
                    ...
                    $('.variant_id').each(function(){
                        ...
                        if(!isNaN(variant_id)) {
                            variant_not.push(variant_id);
                        }
                    })
                    if(!checkEmpty(variant_not)) {
                        data.variant_not = variant_not;
                    }
                    loadAjaxPost('{{ route('admin.ajax.products.combo_select') }}', data, {
                        beforeSend: function(){},
                        success:function(result){
                            if (result.status == 1) {
                                list = '';
                                $.each(result.data, function(index, item) {
                                    list += `<li class="suggest-item" data-suggest_sussget_name data-id="${item.id}" data-vid="${item.variant_id}" data-reguler_price="${item.reguler_price}" data-sale_price="${item.sale_price}">${item.name}</li>`;
                                });
            ...
            html = '<div class="result-full-item col col-md-12">'+
                        ...
                        '<div class="row">'+
                            '<div class="controls col col-md-6 col-lg-6 col-xs-6 suggest">'+
                                ...
                                '<input type="hidden" class="variant_id" name="sale['+n+'][variant_id]" value="'+variant_id+'">'+
                                '<div class="suggest-result"><ul></ul></div>'+
                            '</div>'+
                            '<div class="controls col col-md-6 col-lg-6 col-xs-6">'+
