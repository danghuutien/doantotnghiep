    ...
    @foreach($stores as $store)
        <div class="list-item store-item">
            <div class="list-item__image">
                <a href="{{ $store->getUrl() }}" class="thumbnail">
                    @include('Default::general.components.image', [
                        'src' => resizeWImage($store->image, 'w400'),
                        'width' => '380px',
                        'height' => '280px',
                        'lazy'   => true,
                        'title'  => $store->name ?? ''
                ...
                @if(count($store->getSlides()))
                    <div class="gallery">
                        <p class="gallery_item" data-src="{{ resizeWImage($store->image, 'w400') }}">
                            @include('Default::general.components.image', [
                                'src' => resizeWImage($store->image, 'w100'),
                                'width' => '50px',
                                'height' => '50px',
                                'lazy'   => true,
                                'title'  => $store->name ?? ''
                        ...
                        @foreach($store->getSlides() as $key => $slide)
                            @if($key < 5)
                                <p class="gallery_item" data-src="{{ resizeWImage($slide, 'w400') }}">
                                    @include('Default::general.components.image', [
                                        'src' => resizeWImage($slide, 'w100'),
                                        'width' => '50px',
                                        'height' => '50px',
                                        'lazy'   => true,
                                        'title'  => ''
            ...
            <div class="list-item__info">
                <h3 class="name">
                    <a href="{{ $store->getUrl() }}" class="fs-20 lh-28 f-w-b">{{ $store->getName() }}</a>
                </h3>
                <p class="address mt-10">
                    ...
                    <span class="fs-16 lh-24 ml-6">{{ $store->getAddress() }}</span>
                </p>
                <p class="phone mt-8">
                    ...
                    <span class="fs-16 lh-19 ml-6">{{ $store->phone }}</span>
                </p>
                @if($store->p_driver == 1)
                    <p class="parking-lot mt-8">
                        ...
                        <span class="fs-16 lh-19 ml-6">Có chỗ đậu xe </span>
                    </p>
                @endif
                <a href="{{ $store->getUrl() }}" class="see_more fs-16 lh-19 mt-14">Xem chi tiết cửa hàng</a>
            </div>
        </div>
    @endforeach
@endif
